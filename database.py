...
class db:
  ...
  def add_one_subscription(self, channel_id, gener, how_many):
    ...
    conn = sqlite3.connect(('database.db'))
    c = conn.cursor()   # create a cursor
    c.execute("SELECT * FROM '%s' WHERE channel_id = (?) AND gener = (?)" % self.table_name, (channel_id, gener))
    existing_data = c.fetchall()
    print('existing_data:',existing_data)
    if existing_data == []:
      # adding new data
      message = f'adding new data: channel_id:{channel_id}, gener:{gener}, how_many:{how_many}'
      c.execute("INSERT INTO '%s' VALUES (?, ?, ?)"%self.table_name, ((str(channel_id)), gener, str(how_many)))
    else:
      # updating existing data
      message = f'updating existing data: channel_id:{channel_id}, gener:{gener}, how_many:{how_many}'
      c.execute("""UPDATE '%s' SET gener = (?), how_many=(?) WHERE channel_id = (?)""" % self.table_name, (gener, str(how_many), channel_id,))
    # data = c.fetchall()
    conn.commit() # commit our command
    conn.close()  # close our connection
    return message
  ...
  def remove_one_subscription(self, channel_id, gener):
    ...
    c.execute("DELETE FROM '%s' WHERE channel_id = (?) AND gener = (?) " % self.table_name, (channel_id, gener,))
  ...
  def get_one(self, channe_id):
    ...
    if self.table_name=='count' or self.table_name=='chain_word':
      c.execute("SELECT * FROM '%s' WHERE channel_id = (?)" % self.table_name, (channe_id,))
      data = {}
  ...
  def add_one_ioe_notificaiton(self, channel_id):
    ...
    c.execute("INSERT INTO '%s' VALUES (?)"%self.table_name, ((str(channel_id)),))
    data = c.fetchall()
  ...
  def remove_one_ioe_notificaiton(self, channel_id):
    ...
    c.execute("DELETE FROM '%s' WHERE channel_id = (?) " % self.table_name, (channel_id,))
  ...
  def add_one_chain_word(self, channel_id, current_word, current_author, current_score, highest_score):
    ...
    conn = sqlite3.connect(('database.db'))
    c = conn.cursor()   # create a cursor
    c.execute("SELECT * FROM '%s' WHERE channel_id = (?)" % self.table_name, (channel_id,))
    existing_data = c.fetchall()
    ...
    if existing_data == []:
      ...
      c.execute("INSERT INTO '%s' VALUES (?, ?, ?, ?, ?)"%self.table_name, ((str(channel_id)), current_word, current_author, current_score, highest_score))
    else:
      # updating existing data
