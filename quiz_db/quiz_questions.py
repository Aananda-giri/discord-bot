...
class QuizQuestions:
  ...
  @staticmethod
  def add_question(server_id, channel_id, question_id, question, options,
                   answer_index, leaderboard_message_id, created_datetime,
                   expire_date_time, users_reacted, correctly_answered_users,
                   active):
  ...
  def get_question(question_id=None):
    ...
    if question_id:
      c.execute("SELECT * FROM quiz_questions WHERE question_id = ?",
                (question_id, ))
      ...
      if result:
        # result = list(result)
        keys = [
            'server_id', 'channel_id', 'question_id', 'question', 'options',
            'answer_index', 'leaderboard_message_id', 'created_datetime',
            'expire_date_time', 'users_reacted', 'correctly_answered_users',
            'active'
        ]
        ...
        result['created_datetime'] = datetime.strptime(
            result['created_datetime'], '%Y-%m-%dT%H:%M:%S')
        result['expire_date_time'] = datetime.strptime(
            result['expire_date_time'], '%Y-%m-%dT%H:%M:%S')

        # Convert JSON strings back to lists
        result['options'] = json.loads(result['options'])
        result['users_reacted'] = json.loads(result['users_reacted'])
        result['correctly_answered_users'] = json.loads(
            result['correctly_answered_users'])
      ...
      if results:
        for result in results:
          results_new.append(dict(zip(keys, result)))
        # for result in results:

        #   result_dict = {}
        #   for n, key in enumerate(keys):
        #     result_dict[key] = result[n]
        #   results_new.append(result_dict)
      return results_new
  ...
  def update_question(question_id, correctly_answered_users, users_reacted):
    ...
    c.execute(
        "UPDATE quiz_questions SET correctly_answered_users = ?, users_reacted = ? WHERE question_id = ?",
  ...
  def deactivate_question(question_id):
    ...
    c.execute("UPDATE quiz_questions SET active = 0 WHERE question_id = ?",
              (question_id, ))
...
if __name__ == "__main__":
  ...
  QuizQuestions.update_question(question_id, new_question, new_options,
                                new_answer)
