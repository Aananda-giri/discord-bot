...
class QuizScores:
  ...
  def add_score(server_id, user_name, score):
    ...
    c = conn.cursor()
    c.execute(
        f"""INSERT INTO quiz_scores (
            server_id, user_name, score
        ) VALUES (?, ?, ?)""", (server_id, user_name, score))
  ...
  def get_score(server_id=None, user_name=None):
    ...
    if server_id == None or user_name == None:
      ...
      return result
    c.execute(
        "SELECT score FROM quiz_scores WHERE server_id = ? AND user_name = ?",
        (server_id, user_name))
    result = c.fetchone()
  ...
  @staticmethod
  def get_top_ten(server_id, how_many=10):
    print(f'------------get_top_ten: {server_id}---------------10-------\n')
    conn = sqlite3.connect(os.path.join(parent_dir, 'quiz_sqlite_db.db'))
    c = conn.cursor()
    c.execute(
        "SELECT * FROM quiz_scores WHERE server_id = ? ORDER BY score DESC LIMIT ?",
        (
            server_id,
            how_many,
        ))
  ...
  def exists(server_id, user_name, check_server_only=False):
    ...
    if check_server_only:
      c.execute("SELECT COUNT(*) FROM quiz_scores WHERE server_id = ?",
                (server_id, ))
      result = c.fetchone()
      conn.close()
    else:
      ...
      c.execute(
          "SELECT COUNT(*) FROM quiz_scores WHERE server_id = ? AND user_name = ?",
          (server_id, user_name))
      result = c.fetchone()
  ...
  def update_scores(server_id, user_name, new_score):
    ...
    if QuizScores.exists(server_id, user_name):
      c.execute(
          "UPDATE quiz_scores SET score = ? WHERE server_id = ? AND user_name = ?",
          (new_score, server_id, user_name))
    else:
      c.execute(
          "INSERT INTO quiz_scores (server_id, user_name, score) VALUES (?, ?, ?)",
          (server_id, user_name, new_score))
