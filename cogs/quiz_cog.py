...
class Quiz(commands.Cog, name="quiz"):
  ...
  @commands.hybrid_command(name="quiz",
                           ...
                           hidden=True,
                           usage='.quiz')
  async def quiz(self, context, question: str, option1: str, option2: str,
                 option3: str, option4: str, answer_index: str):

    print(
        f"question: \"{question}\" options: {option1, option2, option3, option4} answer_index:{answer_index}"
    )
    options = [option1, option2, option3, option4]

    question_message = await context.send(embed=get_question_embed(
        question, options),
                                          silent=False)
    ...
    leaderboard_data = QuizScores.get_top_ten(server_id, how_many=15)
    print(f'\n\n------------- L:{leaderboard_data}-----------------\n\n')
    # leaderboard_data = [('cnon', {'score': 6}), ('bnon', {'score': 3}), ('enon', {'score': 3}), ('dnon', {'score': 2}), ('fnon', {'score': 2}), ('anon', {'score': 6})]
    # leaderboard_data =
    leaderboard_message = await context.send(
        embed=create_stylish_leaderboard_embed(leaderboard_data), silent=False)

    # save question
    expire_datetime = question_message.created_at + datetime.timedelta(
        hours=3)  # Expires after 3 hours minutes
    ...
    QuizQuestions.add_question(server_id,
                               ...
                               expire_date_time=expire_datetime,
                               users_reacted=[],
  ...
  async def quiz(self, context, question, *options):
    ...
    print(
        f"question: \"{question}\" options: {options} answer_index: {answer_index}"
    )
    # options = [option1, option2, option3, option4]
    ...
    # save question
    expire_datetime = question_message.created_at + datetime.timedelta(
        hours=24)  # Expires after 24 hours minutes
    # print(server_id, question_message.id, question, options, int(answer_index), leaderboard_message.id, question_message.created_at, expire_datetime, [], [], True)
    print(
