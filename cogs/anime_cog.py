#source: https://github.com/Der-Eddy/discord_bot/blob/master/cogs/anime.py
# tutorial: https://www.youtube.com/watch?v=CGQo7IiMDI4
# reference: https://animec.readthedocs.io/en/latest/charsearch.html
...
import random, re, aiohttp, discord, animec, datetime
from discord.ext import commands

class anime(commands.Cog):
    '''Alles rund um Animes => Everything about anime'''
...
@commands.hybrid_command(aliases=['headpat'])
    async def pat(self, ctx, member: discord.Member = None):
        ...
        if member == ctx.me:
            print('\nmember is me\n')
            msg = f'thank you {ctx.author.mention} <:Hiding:322410632517517324> \n{random.choice(gifs)}'
  ...
  '''
    ...
    async def ratewaifu(self, ctx, *, waifuName: str):
        ...
        msg = f'{emoji} I evaluate **{waifuName}** als **{rating}/10**'
    ...
    async def saucenao(self, ctx, url: str = None):
        ...
        Beispiel:
        Try to find the source of an anime picture
        example
        -----------
        :saucenao
        ...
        if url == None:
            ...
            await ctx.send(':x: Incorrect URL specified!')
            return

        tmp = await ctx.send(f'Try the source of the image <{url}> to find ...')
    ...
    async def anime(self, ctx, *query):
        ...
        try:
            ...
            await ctx.send(embed = discord.Embed(description='no corresponding anime found for the search query :{}'.format(ex), color=discord.Color.red()))
            return
    ...
    async def aninews(self, ctx, amount:int=3):
        ...
        for i in range(amount):
            embed.add_field(name = f"{i+1} {titles[i]}", value=f"{description[:200]}...\n[Read more]({links[i]})", inline=False)
        await ctx.send(embed=embed)
